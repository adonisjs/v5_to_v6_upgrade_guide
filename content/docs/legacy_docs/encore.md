...
:::warning
We recommend migrating to Vite as soon as possible. See [migration guide](../other/vite_migration.md)
:::
...
2. Create a `webpack.config.cjs` file

3. Install needed dependencies (`@babel/core`, `@babel/preset-env`, `@symfony/webpack-encore`, `webpack`, `webpack-cli` )
...
Similarly, the `node ace build` command will also run the `encore production` command to bundle the frontend assets alongside your AdonisJS build.
